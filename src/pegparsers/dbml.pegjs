...
schema
  = (tableGroupDefinition / tableDefinition / projectDefinition / enumDefinition / refDefinition / nothingDefinition)*

tableDefinition
  = "Table"i _ name:PropertyName _ "as"i _ alias:PropertyName _ attrs:attrs? _ "{" _ rows:tableRow* _ "}" { return { type: "Table", name, alias, attrs, rows }; }
  / "Table"i _ name:PropertyValue _ attrs:attrs? _ "{" _ rows:tableRow* _ "}" { return { type: "Table", name, alias: null, attrs, rows }; }

tableRow
  = name:PropertyName _ datatype:PropertyValue _ attrs:attrs? _? { return { type: "TableColumn", name, datatype, attrs }; }
  / item: attrItem _? { return item }

projectDefinition
  = "Project"i _ name:PropertyValue  _ "{" _ rows:prohectRow* _ "}" { return { type: "Project", name, rows }; }

prohectRow
  = item: attrItem _? { return item }

enumDefinition
  = "Enum"i _ name:PropertyName _ "{" _ rows:enumRow* _ "}" { return { type: "Enum", name, rows }; }

enumRow
  = name:PropertyName _ attrs:attrs? _? { return { type: "EnumValue", name, attrs }; }

refDefinition
  = "Ref"i _ name:PropertyName _ ":" _ ref:refPath { return { type: "Ref", name, ref }; }
  / "Ref:"i _ ref:refPath { return { type: "Ref", name: null, ref }; }
  / "Ref"i _ name:PropertyName _ "{" _ ref: refPath _ "}" { return { type: "Ref", name, ref }; }
...
refPath
  = left:PropertyName _ direction:("<>" / "<" / ">" / "-") _ right:PropertyName _ attrs:attrs { return { left, direction, right, attrs }; }
  / left:PropertyName _ direction:("<>" / "<" / ">" / "-") _ right:PropertyName { return { left, direction, right, attrs: null }; }

tableGroupDefinition
  = "TableGroup" _ name:PropertyName _ "{" _ rows:tableGroupRow* _ "}" { return { type: "TableGroup", name, rows }; }

tableGroupRow
  = name:PropertyName _? { return { type: "TableGroupName", name }; }

nothingDefinition
  = (WhiteSpace / LineTerminatorSequence / Comment)+ { return text() }

attrs
  = "[" _ attribute:attrsList _ "]" _? { return attribute; }

attrsList
  = head:attrItem tail:(_ "," _ attrItem)* {
      return buildList(head, tail, 3);
    }

attrItem
  = key:PropertyName _ ":" _ value:PropertyValue { return { type: "AttrKV", key: key, value: value }; }
  / key:PropertyName _ "{" _ value:[^}]+ "}" { return { type: "AttrBlock", key: key, value: text() }; }
